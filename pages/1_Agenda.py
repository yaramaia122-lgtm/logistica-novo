...
# Pré-tratamento para o Gerenciador de Status funcionar
...
# 🌟 5. GERENCIAMENTO DE CANCELAMENTOS / STATUS
st.markdown('<div class="section-header">Gerenciamento de Viagens (Cancelar / Ocultar)</div>', unsafe_allow_html=True)

lista_g = []
for i, row in df_v.iterrows():
    if str(row.get('passageiro', '')).strip() != "":
        lista_g.append(f"{i} - {row.get('passageiro', '')} ({row.get('data', '')}) - {row.get('trajeto', '')} [{row.get('status', 'Confirmado')}]")

col_s, col_st = st.columns([3, 1])
with col_s:
    v_sel = st.selectbox("Selecione o passageiro/viagem para alterar:", options=[""] + lista_g)
with col_st:
    n_st = st.selectbox("Novo status:", ["Confirmado", "Cancelado", "Ocultado"])
...
if st.button("🔄 Atualizar Status", width='stretch'):
    if v_sel:
        ...
        st.success(f"Viagem alterada para '{n_st}' com sucesso! Ela não aparecerá mais no PDF.")
        st.rerun()
    else:
        st.warning("Por favor, selecione uma viagem na lista.")

st.markdown("---")

# 6. DADOS DE VIAGEM (Filtra Apenas Confirmados para a Tela e PDF)
...
# 7. GERADOR DE HTML PERSONALIZADO (Com Bordas e Títulos Agrupados)
...
# 8. EXIBIÇÃO NO PAINEL COM AS COLUNAS AGRUPADAS NATIVAS
