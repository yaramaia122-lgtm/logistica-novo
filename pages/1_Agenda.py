...
try:
    ...
    # Padronização de segurança das colunas em memória
    df_v.columns = df_v.columns.str.strip().str.lower()
    df_o.columns = df_o.columns.str.strip().str.lower()

    # 🛡️ TRATAMENTO CRÍTICO CONTRA O VALUERROR (AMBIGUOUS SERIES)
    # Garante que as duas colunas cruciais de filtragem existam, sejam texto limpo e sem nulos
    ...
    # Função de proteção para evitar o KeyError nas colunas secundárias
    ...
    # Render do Bloco de Observações
    ...
    for index, row in df_o.iterrows():
        ...
        with c_dia: st.markdown(f"<p style='padding-top:15px; font-weight:bold; color:#333333;'>{row['dia']}</p>", unsafe_allow_html=True)
        with c_data: st.markdown(f"<p style='padding-top:15px; color:#555555;'>{row['data']}</p>", unsafe_allow_html=True)
        with c_texto:
            t = st.text_area(label=f"Obs {row['dia']}", value=row['observacao'], key=f"obs_{index}", label_visibility="collapsed")
            novas_obs.append(t)
    ...
    passageiros_selecionados = st.multiselect("Selecione um ou mais passageiros (Deixe vazio para mostrar todos):", options=lista_passageiros)
    ...
    # --- 📄 EXIBIÇÃO DAS TABELAS NA TELA CORRIGIDA SEM AMBIGUIDADE ---
    ...
    df_pl_screen = df_filtrado[df_filtrado['trajeto'].eq("pontes e lacerda x cuiabá")]
    ...
    df_cp_screen = df_filtrado[df_filtrado['trajeto'].eq("cuiabá x pontes e lacerda")]
