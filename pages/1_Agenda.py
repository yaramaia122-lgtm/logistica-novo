...
# Carregamento bruto dos arquivos
...
# 1. TRATAMENTO CRÍTICO DAS OBSERVAÇÕES
...
df_o_edit = st.data_editor(df_o_at, column_config={"dia": st.column_config.TextColumn("Dia", disabled=True), "data": st.column_config.TextColumn("Data", disabled=True), "observacao": st.column_config.TextColumn("Observação", width="large")}, hide_index=True, width='stretch', row_height=100, key="ed_obs_v_final")
...
# 2. PADRONIZAÇÃO COMPLETA DOS DADOS DE VIAGEM (Corrige o histórico antigo)
df_v.columns = df_v.columns.str.strip().str.lower()
df_v.columns = df_v.columns.str.replace("á", "a").str.replace("í", "i").str.replace("º", "")
df_v = df_v.loc[:, ~df_v.columns.duplicated()]

# Garante a existência da coluna status no histórico antigo
