...
# Lista extraída diretamente da sua imagem de referência
CC_LISTA = [
...
with st.form("programar_viagem"):
    ...
    hs = c2.text_input("Hora Saída (ex: 06h00)")
    ht = c2.text_input("Hotel/Destino")
    
    st.markdown("### ✈️ Logística de Voo")
    vn = c1.text_input("Cia/nº voo")
    vh = c2.text_input("Horário do Voo (ex: 15h10)")
    
    st.markdown("### 💰 Custos Ocultos (Restrito Administrativo)")
    ...
    if st.form_submit_button("💥 AGENDAR LOGÍSTICA"):
        ...
        st.success("Integrado à Agenda com sucesso!"); st.rerun()
